...
def load_csv(filepath):
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            # FIX: Rating di CSV pakai koma sebagai desimal ("4,5"), ganti ke titik dulu
            raw_rating = row["Rating"].strip().replace(",", ".")
            raw_tahun  = row["Tahun_Buka"].strip()

            data.append({
                "name":         row["Nama"].strip(),
                "category":     row["Kategori"].strip().lower(),
                "sub_category": row["Sub-Kategori"].strip().lower(),
                ...
                "google_maps":  row["Google_Maps"].strip(),
                "tahun_buka":   int(raw_tahun) if raw_tahun.isdigit() else 0,
            })
    return data
